...
# src imports
# -----------
from src.routing.NetworkBasicReliability import NetworkBasicReliability
...
class NetworkBasicReliabilityWithStore(NetworkBasicReliability):
    ...
    def return_travel_costs_1to1(self, origin_position, destination_position, customized_section_cost_function = None): ##similar to base case, no change
        """
        ...
        if self.travel_time_infos.get( (origin_node, destination_node) ) is not None: #Check for tt infos in DB
            ...
            if s[0] == np.inf: # TODO # seems to be a bug. Do you know what's the problem here?
                ...
                s = R.compute(return_route=False)[0][1] #s ()
    ...
    def return_travel_costs_1to1_reliability(self, origin_position, destination_position, customized_section_cost_function = None):
        ...
        if self.travel_time_infos_reliability.get( (origin_node, destination_node) ) is not None:
            s = self.travel_time_infos_reliability[(origin_node, destination_node)]
            if s[0] == np.inf: # TODO # seems to be a bug. Do you know what's the problem here?
                LOG.warning(f"in return_travel_costs_1to1, travel_time_infos from nodes {origin_node} to {destination_node}"
                         f"yields s={s}")
                R = RouterReliability(self, origin_node, destination_nodes=[destination_node], mode='bidirectional',
                           customized_section_cost_function=customized_section_cost_function)
                s = R.compute(return_route=False)[0][1]
                LOG.warning(f"after recalculation of the routes, s={s}")
        else:
            ...
            s = R.compute(return_route=False)[0][1]
            self._add_to_database_reliability(origin_node, destination_node, s[0], s[1], s[2])
        return (s[0] + origin_overhead[0] + destination_overhead[0], s[1] + origin_overhead[1] + destination_overhead[1], s[2] + origin_overhead[2] + destination_overhead[2])
        
    def _reset_internal_attributes_after_travel_time_update(self):
        self.travel_time_infos = {}
        self.travel_time_infos_reliability = {}
    ...
    def _add_to_database_reliability(self, o_node, d_node, cfv, tt, std):
        """ this function is call when new routing results have been computed
        depending on the class the function can be overwritten to store certain results in the database
        """
        if self.travel_time_infos_reliability.get( (o_node, d_node) ) is None:
            self.travel_time_infos_reliability[ (o_node, d_node) ] = (cfv, tt, std)